...
def mode(data: list) -> list | str:
    ...
    for number in unique_values:
        ...
        if count > 1:
            result.append((number, count))  # add only numbers appeared more than once
    ...
    # all value-count tuple pair with equal number of value appearance in the data
    return [number_count_pair for number_count_pair in result if number_count_pair[1] == result[0][1]]
